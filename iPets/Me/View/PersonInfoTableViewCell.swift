...
class PersonInfoTableViewCell: UITableViewCell {
    ...
    func rebuildCell(){
        //个人信息页-title
        let nameSize = sizeWithText(infoItem.name as NSString, font: settingPageNameFont, maxSize: CGSize(width: Width/2, height: self.frame.height))
        ...
        if(infoItem.TDicon == "" && infoItem.lable == ""){
            ...
            //个人信息页-其他栏
            let lableSize = sizeWithText(infoItem.lable as NSString, font: settingPageLableFont, maxSize: CGSize(width: Width-myName.frame.maxX-50, height: self.frame.height))
