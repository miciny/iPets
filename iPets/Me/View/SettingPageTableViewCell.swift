...
class SettingPageTableViewCell: UITableViewCell {
    ...
    func rebuildCell(){
        ...
        //个人信息页-title
        let nameSize = sizeWithText(settingItem.name as NSString, font: settingPageNameFont, maxSize: CGSize(width: Width/2, height: self.frame.height))
        ...
        if(settingItem.pic != nil || settingItem.lable != nil){
            //设置 帐号
            let lableSize = sizeWithText(settingItem.lable! as NSString, font: settingPageLableFont, maxSize: CGSize(width: Width-myName.frame.maxX-50, height: self.frame.height))
