...
//根据2030，返回一个[20, 30]
func getTag(_ position: Int) -> [Int]{
    let a = position / 100
    let b = position % 100
    return [a, b]
}
...
func getRandomNo(_ min: Int, max: Int) -> Int{
...
//随机一个数
func randomNo(_ min: UInt32, max: UInt32) -> Int{
    return Int(arc4random_uniform(max - min) + min)  //随机到的编号
}

//概率控制
func probabilityController(_ min: Int, max: Int) -> NSMutableArray{
...
//比较器，排序用
let getSort = {(n1: Int, n2: Int) -> Bool in
    //进行从小到大的排序
    return n2 > n1
}
