    ...
    func deleteDirInChatCache(_ dirName: String){
        ...
        if defaultManager.fileExists(atPath: singleChatDir) {
            do{
                ...
                logger.info("删除"+dirName+"缓存文件夹成功！")
            }catch let error as NSError{
                logger.info("删除"+dirName+"缓存文件夹失败！")
                logger.info(error)
            }
        }
    }
    ...
    func createDirInChatCache(_ dirName: String) -> AnyObject{
        ...
        if !manager.fileExists(atPath: singleChatDir) {
            do{
                ...
                logger.info("创建"+dirName+"聊天的子文件夹成功！")
            }catch let error as NSError{
                logger.info("创建"+dirName+"聊天的子文件夹失败！")
                logger.info(error)
            }
        }
    ...
    func savaImageToChatCacheDir(_ dirName: String, image: UIImage, imageName: String) -> Bool {
        ...
        if let imageData = imageData{
            ...
            do{
                ...
                isSaved = true
                logger.info("保存图片"+imageName+"到"+dirName+"目录成功！")
            }catch let error as NSError{
                logger.info("保存图片"+imageName+"到"+dirName+"目录失败！")
                logger.info(error)
                isSaved = false
            }
        }
    ...
    func savaImageToFindPetsCacheDir(_ image: UIImage, imageName: String) -> Bool {
        ...
        if let imageData = imageData{
            ...
            do{
                ...
                isSaved = true
                logger.info("保存图片"+imageName+"到findPets目录成功！")
            }catch let error as NSError{
                logger.info(error)
                logger.info("保存图片"+imageName+"到findPets目录失败！")
                isSaved = false
            }
        }
        return isSaved
        ...
        for i in 0 ..<  imageName.count{
            ...
            if manager.fileExists(atPath: imagePath) {
                do{
                    ...
                    logger.info("从findpets删除高清图成功")
                }catch let error as NSError{
                    logger.info("从findpets删除高清图失败！")
                    logger.info(error)
                }
            ...
            if manager.fileExists(atPath: imagePath1) {
                do{
                    ...
                    logger.info("从findpets删除普通图成功！")
                }catch let error as NSError{
                    logger.info("从findpets删除普通图失败！")
                    logger.info(error)
                }
