...
class SendFindMyPetsInfoTableViewCell: UITableViewCell {
    ...
    func rebuildCell(){
        ...
        if(sendInfoItem.name != ""){
            ...
            //普通设置栏-title
            let titleSize = sizeWithText(sendInfoItem.name as NSString, font: sendPageNameFont, maxSize: CGSize(width: Width/2, height: infoIcon.frame.height))
            ...
            //普通设置栏-lable
            let lableSize = sizeWithText(sendInfoItem.lable as NSString, font: sendPageLableFont, maxSize: CGSize(width: Width/2, height: infoIcon.frame.height))
