...
class SettingTableViewCell: UITableViewCell {
    ...
    func rebuildCell(){
        if(settingItem.lable == ""){
            ...
            //个人设置栏-寻宠昵称
            let nicknameSize = sizeWithText("寻宠号："+settingItem.nickname, font: settingPageLableFont, maxSize: CGSize(width: Width/2, height: myIcon.frame.height/2))
        ...
        if(settingItem.name == ""){
            ...
            self.addSubview(myIcon)
            //普通设置栏-title
            let lableSize = sizeWithText(settingItem.lable, font: settingPageNameFont, maxSize: CGSize(width: Width/2, height: myIcon.frame.height))
